...
class MYSTICPredictor:
    ...
    def assess_risk(self, phi_result: Dict, attractor_result: Dict, 
        ...
        if hasattr(self, 'current_time_series'):
            # Calculate changes from current time series
            ts = self.current_time_series
            if len(ts) > 2:
                changes = [ts[i+1] - ts[i] for i in range(len(ts)-1)]
                avg_change = sum(changes) // len(changes) if changes else 0
                max_change_negative = max((c for c in changes if c < 0), default=0)  # Most negative change
                
                # Strong negative trend indicates rapidly dropping pressure (dangerous)
                if avg_change < -3:  # Reduced threshold for more sensitivity
                    if attractor_result["classification"] in ["WATCH", "STORM"]:
                        risk_score += 25  # Additional boost for pressure drops in warning categories
                    elif attractor_result["classification"] in ["CLEAR", "STEADY_RAIN"]:
                        risk_score += 35  # Even bigger boost when classification might miss the danger
                if max_change_negative < -5:  # Reduced threshold for more sensitivity
                    risk_score += 15  # Sharp drop is dangerous regardless of classification
        ...
        if hasattr(self, 'current_time_series') and len(self.current_time_series) > 5:  # Need sufficient data
...
def validate_system_accuracy():
    ...
    for case in validation_cases:
        ...
        accuracy_indicator = "✓" if (
            ...
            (expected_risk == "CRITICAL" and predicted_risk in ["HIGH", "CRITICAL"])
        ) else "✗"
